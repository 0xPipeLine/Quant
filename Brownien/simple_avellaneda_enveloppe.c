...
int tester_init(Tester *t, double capital, int n_steps){
    ...
    if(portfolio_init(&t->portfolio_envelope, capital, MAX_ORDERS) != 0) return -1;
...
void envelope_strategy(Market *m, Portfolio *p, int levels, double min_spread, double max_spread, int sma_period) {
    if (levels < 2) levels = 2; // Minimum 2 niveaux
...
int main(){
    ...
    printf("Demarrage des strategies de trading...\n");
    
    simple_mm(&t.market, &t.portfolio_simple, 0.001);
    avellaneda(&t.market, &t.portfolio_avellaneda, 0.1, 1.5);
    envelope_strategy(&t.market, &t.portfolio_envelope, 4, 0.0005, 0.008, 9);
    ...
    double final_envelope = portfolio_value(&t.portfolio_envelope, t.market.prices[t.market.n-1]);
    ...
    printf("Envelope: %.2f (volume %.2f)\n", final_envelope, t.portfolio_envelope.volume);
